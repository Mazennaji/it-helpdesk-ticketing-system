...
const selectCls = "w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400/40 disabled:bg-white/[0.02] disabled:text-slate-500 [&>option]:bg-[#0E1B33]";
...
export default function TicketDetail() {
  ...
  const loadAll = () => {
    ...
    return Promise.all(calls).then((results) => {
      ...
      setForm({
        ...
        categoryId: t.category.id, priorityId: t.priority.id, statusId: t.status.id,
      });
    });
  };
  ...
  const handleAssign = async (e) => {
    ...
    try {
      setTicket(await assignTicket(id, agentId));
      setActivity(await fetchActivityLog(id));
  ...
  if (loading) {
    return <AppLayout title="Ticket Detail"><div className="text-slate-500 text-sm">Loading ticket...</div></AppLayout>;
  }
  if (error && !ticket) {
    return <AppLayout title="Ticket Detail"><div className="text-red-400 text-sm">{error}</div></AppLayout>;
  }

  const tabBtn = (key, label) => (
    <button onClick={() => setActiveTab(key)}
      className={`px-5 py-3 text-sm font-medium border-b-2 -mb-px transition-colors ${
        activeTab === key ? "border-blue-400 text-white" : "border-transparent text-slate-400 hover:text-slate-200"
      }`}>
      {label}
    </button>
  );
  ...
  return (
    <AppLayout title={ticket.referenceNo} subtitle={ticket.title}>
      <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-6 mb-5">
        <StatusTimeline currentStatus={ticket.status.name} />
      </div>

      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-6">
            {error && (
              <div className="mb-5 rounded-md bg-red-500/10 border border-red-500/20 px-3 py-2 text-sm text-red-300">{error}</div>
            )}
          ...
          <div className="rounded-xl border border-white/8 bg-[#0E1B33]">
                    ...
                    <div key={c.commentId} className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {c.isInternal && (
                            <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-300">Internal note</span>
                          )}
                ...
                <form onSubmit={handlePostComment} className="border-t border-white/8 pt-4">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <button type="submit" disabled={postingComment}
                      className="bg-white text-[#0B1F3A] text-sm font-medium px-4 py-2 rounded-lg hover:shadow-[0_0_24px_rgba(59,130,246,0.35)] disabled:opacity-60 transition-all">
            ...
            {activeTab === "attachments" && (
              <div className="p-6">
                {uploadError && (
                  <div className="mb-4 rounded-md bg-red-500/10 border border-red-500/20 px-3 py-2 text-sm text-red-300">{uploadError}</div>
                )}
                ...
                <div className="border-t border-white/8 pt-4">
                  ...
                  <label htmlFor="attachment-upload"
                    className="inline-block cursor-pointer border border-white/15 text-slate-200 text-sm font-medium px-4 py-2 rounded-lg hover:bg-white/5 transition-colors">
        ...
        <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-6 h-fit">
          <h2 className="text-sm font-semibold text-white mb-4">Ticket Info</h2>
          <dl className="space-y-3 text-sm mb-5">
            <div className="flex justify-between">
              <dt className="text-slate-500">Created by</dt>
              <dd className="text-slate-200 font-medium">{ticket.createdByName}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-500">Created</dt>
              <dd className="text-slate-200 font-medium">{new Date(ticket.createdAt).toLocaleString()}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-slate-500">Last updated</dt>
              <dd className="text-slate-200 font-medium">{new Date(ticket.updatedAt).toLocaleString()}</dd>
            </div>
            {ticket.resolvedAt && (
              <div className="flex justify-between">
                <dt className="text-slate-500">Resolved</dt>
                <dd className="text-slate-200 font-medium">{new Date(ticket.resolvedAt).toLocaleString()}</dd>
              </div>
            )}
          </dl>
          ...
          <div className="pt-1 border-t border-white/8">
            {!confirmDelete ? (
              <button onClick={() => setConfirmDelete(true)} className="text-sm font-medium text-red-400 hover:text-red-300 mt-4">
