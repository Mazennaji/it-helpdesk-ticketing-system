...
const selectCls = "w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400/40 [&>option]:bg-[#0C1426]";
...
export default function CreateTicket() {
  ...
  useEffect(() => {
    Promise.all([fetchCategories(), fetchPriorities()]).then(([c, p]) => {
      setCategories(c); setPriorities(p);
      setForm((f) => ({ ...f, categoryId: f.categoryId || c[0]?.id || "", priorityId: f.priorityId || p[0]?.id || "" }));
    });
  }, []);
  ...
  const handleSuggest = async () => {
    ...
    try {
      ...
      if (!s.categoryId) { setAiHint("Couldn't confidently categorize this yet. Add more detail or set it manually."); return; }
      setForm((f) => ({ ...f, categoryId: s.categoryId, priorityId: s.priorityId ?? f.priorityId }));
      const confident = s.confidence >= 0.7;
      setAiHint(isStaff && confident
        ? `Auto-filled: ${s.category} / ${s.priority}. ${s.reasoning || ""}`
        : `Suggested: ${s.category} / ${s.priority}. ${s.reasoning || ""} You can change these before submitting.`);
    } catch {
        ...
        <GlassCard className="p-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" name="title" required value={form.title} onChange={handleChange} placeholder="Short summary of the issue" className={inputCls} />
            </div>
            ...
            <div>
              ...
              {aiHint && (<p className="mt-2 text-xs text-slate-300 bg-blue-500/10 border border-blue-500/20 rounded-md px-3 py-2">{aiHint}</p>)}
            </div>
