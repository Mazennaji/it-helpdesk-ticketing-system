...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchReportSummary(), fetchReportTrend(range), fetchReportByCategory(), fetchReportByPriority()])
      .then(([s, t, c, p]) => { if (!active) return; setSummary(s); setTrend(t); setByCategory(c); setByPriority(p); })
      .catch(() => {})
      ...
      <div ref={rootRef} className="relative z-10">
          ...
          <div className="inline-flex rounded-lg border border-white/10 bg-white/[0.03] p-1">
            {RANGE_OPTIONS.map((opt) => (
              <button key={opt.value} onClick={() => setRange(opt.value)}
                className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${range === opt.value ? "bg-white text-[#0B1F3A]" : "text-slate-400 hover:text-white"}`}>
                {opt.label}
              </button>
            ))}
          </div>
          <button onClick={() => downloadCsv(`ticket-trend-${range}d.csv`, trend)}
            className="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-medium text-slate-200 glass hover:border-white/15 rounded-lg transition-colors">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
          {summaryCards.map((stat, i) => (
            <div key={stat.key} ref={(el) => (cardRefs.current[i] = el)} className="glass glass-hover-lift sheen p-4 relative overflow-hidden">
              <div className="absolute inset-x-0 top-0 h-px opacity-70" style={{ background: `linear-gradient(90deg, transparent, ${stat.accent}, transparent)` }} />
              ...
              <p className="text-2xl font-semibold text-white tracking-tight tabular-nums">{summary?.[stat.key] ?? 0}</p>
            </div>
          ))}
        </div>

        <div ref={trendRef}>
          <GlassCard glow className="p-7 mb-4">
            <SectionHeading eyebrow={`${totalTickets} tickets`} title="Ticket Volume Over Time"
              right={<span className="text-xs text-slate-500 font-mono">{RANGE_OPTIONS.find((o) => o.value === range)?.label}</span>} />
            <div style={{ width: "100%", height: 300 }}>
              <ResponsiveContainer>
                <AreaChart data={trend}>
                  ...
                  <Area type="monotone" dataKey="count" stroke="#60A5FA" strokeWidth={2.5} fill="url(#repVol)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
        ...
        <div ref={breakdownRef} className="grid lg:grid-cols-2 gap-4">
          <GlassCard className="p-7">
            <SectionHeading eyebrow="Breakdown" title="Tickets by Category"
              right={<button onClick={() => downloadCsv("tickets-by-category.csv", byCategory)} className="text-xs font-medium text-blue-400 hover:text-blue-300">Export</button>} />
            <div style={{ width: "100%", height: 260 }}>
              <ResponsiveContainer>
                <BarChart data={byCategory} layout="vertical" margin={{ left: 20 }}>
                  ...
                  <Bar dataKey="count" fill="url(#repBar)" radius={[0, 6, 6, 0]} barSize={16} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-7">
            <SectionHeading eyebrow="Distribution" title="Tickets by Priority"
              right={<button onClick={() => downloadCsv("tickets-by-priority.csv", byPriority)} className="text-xs font-medium text-blue-400 hover:text-blue-300">Export</button>} />
            <div style={{ width: "100%", height: 260 }}>
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={byPriority} dataKey="count" nameKey="label" innerRadius={55} outerRadius={85} paddingAngle={3} stroke="#0C1426" strokeWidth={2}>
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-2">
              {byPriority.map((entry) => (
                <span key={entry.label} className="flex items-center gap-1.5 text-xs text-slate-400">
                  ...
                  {entry.label} <span className="tabular-nums text-slate-500">{entry.count}</span>
                </span>
              ))}
            </div>
          </GlassCard>
