...
export default function TicketList() {
  ...
  const [filters, setFilters] = useState({
    search: searchParams.get("q") || "", categoryId: "", priorityId: "", statusId: "",
  });
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(statRefs.current, { opacity: 0, y: 22 });
      gsap.set(tableRef.current, { opacity: 0, y: 24 });

      const tl = gsap.timeline({ defaults: { ease: "power3.out" } });
      tl.to(statRefs.current, { opacity: 1, y: 0, duration: 0.55, stagger: 0.09 })
        .to(filterBarRef.current, { opacity: 1, y: 0, duration: 0.5 }, "-=0.25")
        .to(tableRef.current, { opacity: 1, y: 0, duration: 0.55 }, "-=0.25");
      ...
      valueRefs.current.forEach((el, i) => {
        ...
        gsap.to(counter, {
          val: to, duration: 0.9, ease: "power2.out",
  ...
  const onRowEnter = (el) => gsap.to(el, { backgroundColor: "rgba(255,255,255,0.035)", duration: 0.15 });
  const onRowLeave = (el) => gsap.to(el, { backgroundColor: "rgba(0,0,0,0)", duration: 0.15 });

  const selectCls = "rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400/40 [&>option]:bg-[#0C1426]";
    ...
    <AppLayout title="Tickets" subtitle="All support requests across your queue">
      <div ref={rootRef} className="relative z-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          {statCards.map((s, i) => (
            <div key={s.key} ref={(el) => (statRefs.current[i] = el)}
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="w-2 h-2 rounded-full glow-dot" style={{ color: s.accent, backgroundColor: s.accent }} />
              </div>
        ...
        <div ref={filterBarRef} className="glass p-5 mb-4 flex flex-wrap gap-3 items-center">
          ...
          <Link to="/tickets/new"
            className="ml-auto sheen bg-white text-[#0B1F3A] text-sm font-medium px-4 py-2 rounded-lg hover:shadow-[0_0_28px_rgba(59,130,246,0.45)] transition-all inline-block">
            + New Ticket
          </Link>
        </div>

        <div ref={tableRef} className="glass overflow-hidden premium-scroll">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && (<tr><td colSpan={8} className="px-5 py-12 text-center text-slate-500">Loading tickets...</td></tr>)}
              {!loading && error && (<tr><td colSpan={8} className="px-5 py-12 text-center text-red-400">{error}</td></tr>)}
              {!loading && !error && tickets.length === 0 && (<tr><td colSpan={8} className="px-5 py-12 text-center text-slate-500">No tickets found.</td></tr>)}
              {!loading && !error && tickets.map((t) => {
                ...
                return (
                  <tr key={t.ticketId} className="border-b border-white/5 last:border-0"
                    onMouseEnter={(e) => onRowEnter(e.currentTarget)} onMouseLeave={(e) => onRowLeave(e.currentTarget)}>
                    <td className="px-5 py-3.5">
                      <Link to={`/tickets/${t.ticketId}`} className="font-mono text-blue-400 hover:text-blue-300 hover:underline">{t.referenceNo}</Link>
                    </td>
                    <td className="px-5 py-3.5 text-slate-200">{t.title}</td>
                    <td className="px-5 py-3.5 text-slate-400">{t.category}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: sc.bg, color: sc.text }}>{t.status}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      {(() => {
                        const sla = t.sla
                          ? { state: t.sla.state, dueAt: new Date(t.sla.dueAt), targetHours: t.sla.targetHours, hoursRemaining: t.sla.hoursRemaining, isResolved: t.sla.state === "Met" || t.sla.state === "Missed" }
                          : evaluateSla(t);
                    ...
                    <td className="px-5 py-3.5 text-slate-400">{t.assignedToName || "Unassigned"}</td>
