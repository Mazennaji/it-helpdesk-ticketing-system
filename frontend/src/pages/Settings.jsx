...
function Banner({ kind, message, onClose }) {
  ...
  return (
    <div
      className={`flex items-start justify-between gap-3 rounded-lg border px-4 py-3 text-sm mb-5 ${styles}`}
    >
      <span>{message}</span>
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    fetchMyProfile()
      ...
      .catch(() =>
        setBanner({ kind: "error", message: "Could not load your profile." })
      )
  ...
  const flash = (kind, message) => {
    ...
    if (kind === "success")
      setTimeout(() => setBanner({ kind: null, message: "" }), 3500);
  };
    ...
    <AppLayout title="Settings" subtitle="Manage your account and preferences">
      <div ref={rootRef} className="max-w-3xl">
        <Banner
          ...
          onClose={() => setBanner({ kind: null, message: "" })}
        />
        ...
        {loading ? (
          ...
          <div ref={panelRef}>
            ...
            {tab === "preferences" && (
              <div className="bg-white rounded-xl border border-slate-200 p-8 divide-y divide-slate-100">
                {[
                  {
                    key: "emailNotifications",
                    title: "Email notifications",
                    desc: "Receive ticket updates and assignments by email.",
                  },
                  {
                    key: "inAppNotifications",
                    title: "In-app notifications",
                    desc: "Show the notification bell badge for new activity.",
                  },
                ].map((row) => (
                  <div
                    key={row.key}
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        savePrefs({ ...prefs, [row.key]: !prefs[row.key] })
                      }
                      className={`relative w-11 h-6 rounded-full transition-colors ${
                        prefs[row.key] ? "bg-blue-500" : "bg-slate-300"
                      }`}
                    >
                      <span
