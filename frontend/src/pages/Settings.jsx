...
export default function Settings() {
    ...
    fetchMyProfile()
      .then((data) => {
        ...
        setProfile({
          ...
          department: data.department ?? "", phoneNumber: data.phoneNumber ?? "",
        });
        setPrefs({
  ...
  const saveProfile = async () => {
    setSaving(true);
    try {
      await updateMyProfile({ fullName: profile.fullName, department: profile.department, phoneNumber: profile.phoneNumber });
      flash("success", "Profile updated.");
    } catch (err) {
      flash("error", err.response?.data?.message || "Could not save your profile.");
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      flash("error", err.response?.data?.message || "Could not change your password.");
    } finally { setSaving(false); }
  };
    ...
    <AppLayout title="Settings" subtitle="Manage your account and preferences">
      <div ref={rootRef} className="max-w-3xl">
          ...
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`px-4 py-2.5 text-sm font-medium -mb-px border-b-2 transition-colors ${
                tab === t.key ? "border-blue-400 text-white" : "border-transparent text-slate-400 hover:text-slate-200"
              }`}>
              {t.label}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div ref={panelRef}>
            {tab === "profile" && (
              <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-8 space-y-5">
                ...
                <Field label="Email" hint="Email is managed by your administrator.">
                  <input className={`${inputCls} opacity-60`} value={profile.email} disabled />
                </Field>
                <div className="grid sm:grid-cols-2 gap-5">
                  <Field label="Department">
                    <input className={inputCls} value={profile.department} onChange={(e) => setProfile({ ...profile, department: e.target.value })} />
                  </Field>
                  <Field label="Phone number">
                    <input className={inputCls} value={profile.phoneNumber} onChange={(e) => setProfile({ ...profile, phoneNumber: e.target.value })} />
                  </Field>
                </div>
            ...
            {tab === "security" && (
              <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-8 space-y-5">
                <Field label="Current password">
                  <input type="password" className={inputCls} value={pwd.currentPassword} onChange={(e) => setPwd({ ...pwd, currentPassword: e.target.value })} />
                </Field>
                <div className="grid sm:grid-cols-2 gap-5">
                  <Field label="New password">
                    <input type="password" className={inputCls} value={pwd.newPassword} onChange={(e) => setPwd({ ...pwd, newPassword: e.target.value })} />
                  </Field>
                  <Field label="Confirm new password">
                    <input type="password" className={inputCls} value={pwd.confirmPassword} onChange={(e) => setPwd({ ...pwd, confirmPassword: e.target.value })} />
                  </Field>
                </div>
                <div className="pt-2">
            ...
            {tab === "preferences" && (
              <div className="rounded-xl border border-white/8 bg-[#0E1B33] p-8 divide-y divide-white/8">
                {[
                  ...
                  <div key={row.key} className="flex items-center justify-between py-4 first:pt-0 last:pb-0">
                    ...
                    <button role="switch" aria-checked={prefs[row.key]}
